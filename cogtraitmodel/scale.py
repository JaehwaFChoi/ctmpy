"""cogtraitmodel.scale — 임의의 숙달 앵커 L 에서의 캘리브레이션과 채점.
...
측정 모형이 아니라 시간축 위의 성장 모형으로 쓴다(`cogtraitmodel.growth`).
"""
...
def _check(L):
    ...
    if not np.isfinite(L) or L <= 0:
        raise ValueError(
            ...
            "아니라 성장 모형이며 cogtraitmodel.growth 를 쓴다."
        )
    return L
...
def score_L(Y, alpha, beta, L, gamma=None, **kwargs):
    ...
    Returns
    -------
    dict — `cogtraitmodel.score` 와 같은 키. ``theta``, ``sd``, ``eap`` 가 L 척도.
    """
    from . import score
