"""ctmpy — Cognitive Trait Model 계열의 추정·채점·정보함수 구현.
...
빠른 사용
---------
    >>> import numpy as np, ctmpy
    >>> theta = ctmpy.gen_theta(500, rng=np.random.default_rng(0))
    >>> alpha = np.full(20, 8.0); beta = np.linspace(0.1, 0.9, 20)
    >>> Y = ctmpy.gen_responses(theta, alpha, beta,
    ...                         rng=np.random.default_rng(1))
    >>> fit = ctmpy.bayes_modal_em(Y)          # 문항모수 캘리브레이션
...
`ctmpy.growth` — L -> inf 구성원이 그곳에서 쓰인다.
